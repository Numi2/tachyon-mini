...
fn keys_dir_from_env() -> PathBuf {
    std::env::var("TACHYON_PCD_KEYS_DIR").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from("data/keys"))
}

fn params_store_paths() -> (std::path::PathBuf, std::path::PathBuf) {
    let base = keys_dir_from_env();
...
fn load_or_setup_publisher_params<const MAX_DEG: usize, const MAX_ROOTS: usize>(k: u32) -> Result<ParamsIPA<G1Affine>> {
    if let Err(_e) = std::fs::create_dir_all(keys_dir_from_env()) { /* best-effort */ }
...
mod tests {
    ...
    fn publisher_verify_mismatch_pi_fails() {
        ...
        let proof = prove_poly_publisher::<MAX_DEG, MAX_ROOTS>(k, a_i, p_i, a_next, h_i, 2, [coeffs[0], coeffs[1], coeffs[2], coeffs[3]], [roots[0]], flags).expect("prove");
    ...
    fn publisher_verify_wrong_block_len_fails() {
        ...
        let h_i = pcs::hash_accumulator_a(a_i, Fr::ZERO, p_i, Fr::ZERO);
        let proof = prove_poly_publisher::<MAX_DEG, MAX_ROOTS>(k, a_i, p_i, a_next, h_i, 1, [coeffs[0], coeffs[1], coeffs[2], coeffs[3]], [roots[0]], flags).expect("prove");
