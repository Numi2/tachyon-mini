...
pub struct SpendLinkCore<const DEPTH: usize> {
    pub proving_k: u32,
    pub params: halo2_proofs::ParamsIPA<pasta_curves::vesta::Affine>,
...
impl<const DEPTH: usize> SpendLinkCore<DEPTH> {
    ...
    pub fn verify(
        ...
        let ok = halo2_proofs::plonk::verify_proof::<halo2_proofs::IPACommitmentScheme<pasta_curves::vesta::Affine>, _, _, _>(
            &self.params,
            &self.vk,
            strategy,
...
pub fn prove_spend_link<const DEPTH: usize>(
    ...
    create_proof::<halo2_proofs::IPACommitmentScheme<pasta_curves::vesta::Affine>, _, _, _, _>(
        &core.params,
        &core.pk,
        &[circuit],
        &[&[&[inst_state], &[inst_link]]],
